...
export function daylightPose(item,size){
 const angle=item.rotation*Math.PI/180,n=[Math.sin(angle),Math.cos(angle)],front=size[2]/2+.06;
 const position=[item.x+n[0]*front,item.y+size[1]*.53,item.z+n[1]*front],target=[position[0]+n[0]*2.4,.18,position[2]+n[1]*2.4];
 return {position,target,angle:Math.atan2(size[0]*.7,Math.hypot(position[1]-.18,2.4))};
}
export function windowDaylightSources(home,anchor,catalog,detailedRoomIds){
 const a=catalog.find(a=>a.id==='wooden_window');if(!a)return [];
 const result=[];
 for(const room of home.rooms){if(room.level!==anchor.level||!detailedRoomIds.has(room.id))continue;
  for(const item of room.items){if(item.stored||item.assetId!==a.id)continue;
   ...
   result.push({id:item.id,roomId:room.id,size:a.size,item:renderItem,...daylightPose(renderItem,a.size)});
  }
 }
