...
export function createWindowDaylight(scene){
 ...
 let sources=[];
 function pose(light,source){light.position.fromArray(source.position);light.target.position.fromArray(source.target);light.angle=source.angle;light.shadow.needsUpdate=true;}
 return {
  ...
  preview(id,item){const i=sources.findIndex(s=>s.id===id);if(i<0)return false;const source=sources[i];Object.assign(source,daylightPose(item,source.size));pose(lights[i],source);return true;},
