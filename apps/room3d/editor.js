...
export async function mountHomeEditor(host,{assetBase,initialState,onChange,onBack,storageKey,signal}={}){
 ...
 function tick(now=performance.now()){
  ...
  if(!document.hidden&&now-lastDraw>=frameInterval-.5){
   ...
   settling=controls.update();if(dirty||breathing||drag){renderer.render(scene,camera);renderedFrames++;dirty=false;lastDraw=now}
  }
  inTick=false;if(settling||dirty||drag||animated.length&&!overview&&!reducedMotion&&qualities[quality].motion)wake();
 }
