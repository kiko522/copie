...
function load(asset:string){let task=assets.get(asset);if(!task){task=new GLTFLoader().loadAsync(`${import.meta.env.BASE_URL}room3d/wardrobe/${asset}`).then(g=>{g.scene.updateMatrixWorld(true);return g.scene;}).catch(e=>{assets.delete(asset);throw e;});assets.set(asset,task);}return task;}
...
export async function dressApprovedWardrobe(rig:Rig,value:ApprovedWardrobe,fits:WardrobeFits={}){
 ...
 const fitById=new Map(defs.map(d=>[d.id,fitForGarment(d.id,fits[d.id])]));
 const meshes:T.SkinnedMesh[]=[],resources:Array<{dispose():void}>=[],original=rig.mesh.geometry;
 ...
 try{for(let k=0;k<defs.length;k++){
  ...
  loaded[k].traverse(o=>{if(!(o instanceof T.SkinnedMesh)||!o.name.startsWith(d.prefix))return;found++;
   ...
   for(let i=0;i<p.count;i++){
    q.fromBufferAttribute(p,i).applyMatrix4(o.bindMatrix);q.y*=rig.bodyHeight;
    delta.set(0,0,0);
    for(let j=0;j<4;j++){const old=si.getComponent(i,j),id=remap[old]??0,w=sw.getComponent(i,j),src=sourceRest[old];if(w&&src){delta.x+=(targetRest[id].x-src.x)*w;delta.y+=(targetRest[id].y-src.y*rig.bodyHeight)*w;delta.z+=(targetRest[id].z-src.z)*w;}si.setComponent(i,j,id);}
    q.add(delta);p.setXYZ(i,q.x,q.y,q.z);
    if(n){normal.fromBufferAttribute(n,i).applyMatrix3(normalMatrix);normal.y/=rig.bodyHeight;normal.normalize();n.setXYZ(i,normal.x,normal.y,normal.z);}
   }
   const materials=(Array.isArray(o.material)?o.material:[o.material]).map(m=>{const copy=m.clone();copy.side=T.DoubleSide;resources.push(copy);return copy;});
   ...
   rig.mesh.parent!.add(mesh);mesh.bind(rig.skeleton,rig.mesh.bindMatrix);meshes.push(mesh);resources.push(g);
 ...
 const hems=new Map<string,number>(),tops=new Map<string,number>();for(const mesh of meshes){mesh.geometry.computeBoundingBox();const id=mesh.userData.garmentId;hems.set(id,Math.min(hems.get(id)??Infinity,mesh.geometry.boundingBox!.min.y/rig.bodyHeight));tops.set(id,Math.max(tops.get(id)??-Infinity,mesh.geometry.boundingBox!.max.y/rig.bodyHeight));}
 ...
 const shoeOpening=shoe&&shoe.opening!==undefined&&shoe.opening>0
  ?garmentTransform('shoes',fitById.get(shoe.id)!,rig.bodyHeight,originalBounds.get(shoe.id)!)(new T.Vector3(0,shoe.opening*rig.bodyHeight,0)).y/rig.bodyHeight:0;
 const clipMesh=(mesh:T.SkinnedMesh,covers:(p:T.Vector3)=>boolean)=>{if(!mesh.geometry.groups.length)mesh.geometry.addGroup(0,mesh.geometry.index!.count,0);const clipped=createClothingMask(mesh.geometry,covers);mesh.geometry=clipped;resources.push(clipped);};
 // Socks stay complete as an asset; only their buried section is clipped while shoes are worn.
 if(shoeOpening>0)for(const mesh of meshes.filter(m=>defs.find(d=>d.id===m.userData.garmentId)?.slot==='socks'))clipMesh(mesh,p=>p.y/rig.bodyHeight<shoeOpening-.055);
 ...
 if(shoe&&trousers){
  ...
  const buriedAbove=garmentTransform('bottom',fitById.get(trousers.id)!,rig.bodyHeight,originalBounds.get(trousers.id)!)(new T.Vector3(0,trousers.bootCover!*rig.bodyHeight,0)).y/rig.bodyHeight;
  if(shoeOpening>buriedAbove)for(const mesh of meshes.filter(m=>m.userData.garmentId===shoe.id))clipMesh(mesh,p=>p.y/rig.bodyHeight>buriedAbove);
 }
 const covered=(p:T.Vector3)=>{const x=Math.abs(p.x),y=p.y/rig.bodyHeight,z=p.z;
  return defs.some(d=>{
   ...
   if(d.id.startsWith('fitted-')||['school-swimsuit','qipao','collar-shirt','stand-collar','basic-tee','school-blazer'].includes(d.id)||d.sleeveOnly)return false;
   if(d.slot==='shoes')return shoeOpening>0&&y<shoeOpening-.055;
   if(d.slot==='socks')return y<(tops.get(d.id)??1.0)-.06;
 ...
 const masked=createClothingMask(original,covered);resources.push(masked);rig.mesh.geometry=masked;
 const lift=Math.max(0,...defs.map(d=>(d.groundOffset??0)*rig.bodyHeight)),parent=rig.mesh.parent!;
