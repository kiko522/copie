...
export interface HomeAsset {id:string;name:string;surface:string;size:number[];default:number[];boxes:number[][];url?:string;paintMaterials?:string[];chairSlots?:{id:string;position:number[];rotation:number}[];collection?:string;category?:string;dining?:boolean;appliance?:'fridge';activity?:({kind:string}&(GamingStation|{stations:GamingStation[]}));contact?:{width:number;depth:number;center?:[number,number]};waterable?:boolean;seats?:{id:string;label:string;position:[number,number,number];rotation?:number}[];building?:boolean;support?:SupportSurface&{areas?:SupportSurface[]}}
