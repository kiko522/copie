...
export default function Home3DView({value,onChange,onBack,character,parts:previewParts}:{value?:Home3DState;onChange:(value:Home3DState)=>void;onBack:()=>void;character?:CharacterProfile;parts?:Parts}){
 ...
 return <div className="relative h-full w-full" style={{paddingTop:'var(--chrome-top, 0px)',paddingBottom:'var(--safe-bottom, 0px)',background:'#e8dde7'}}>
   ...
   {savedState&&<CreatorRollBridge request={creatorReady&&extraItems?1:0} savedState={savedState} extraItems={extraItems} onReady={()=>setCreatorReady(true)} onResult={result=>{decodeParts(result).then(setParts).catch(e=>setResidentError(String(e)));}} onError={setResidentError}/>}
