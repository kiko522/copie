...
export const SHOWROOMS={
 ...
 kitchen:{name:'黑白原木厨房',wall:'#e8dfcf',floor:'#72656d',trim:'#40333d',floorStyle:'stone',wallStyle:'tile',items:[['show_kitchen_counter',1.3,-3],['show_kitchen_range',-3.5,-2.4,90],['show_kitchen_island',0,0],['show_kitchen_bench',0,1.55],['kitchen_fridge',-3.4,.1,0,'#34303a'],['kitchen_bin',3.45,2]],door:'arch'},
...
const DECOR={
 ...
 kitchen:{wall:[['suite_window_kitchen',1.3,2.65,-4],['suite_wall_shelf',-4.7,2.6,1.1]],props:[['dining_napkins','show_kitchen_island',-.8,0],['daisy_vase','show_kitchen_island',.8,0],['suite_tea_tray','show_kitchen_island',0,0]]},
...
export function furnishShowroom(room,key,catalog){
 ...
 for(const [assetId,x,z,rotation=0,color=null]of template.items){const a=catalog.find(a=>a.id===assetId);if(!a)throw Error('样板房素材未加载：'+assetId);const item={id:uid(),assetId,x,y:.15,z,rotation,color,stored:false};const reason=placementError(item,room,catalog);if(reason)throw Error(template.name+' · '+a.name+'：'+reason);room.items.push(item);}
