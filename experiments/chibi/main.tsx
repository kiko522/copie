...
function App() {
    ...
    useEffect(()=>{
        ...
        {world?<><section style={{position:'fixed',inset:0,zIndex:10}}>{parts?<Home3DView suspended={editing} hair={effectiveHair} parts={renderParts||parts} value={home} onChange={saveHome} onBack={()=>setWorld(false)}/>:<p>{error||'小人正在搬家…'}</p>}<button disabled={!rollReady||rolling} onClick={()=>setEditing(true)} style={{position:'absolute',left:18,top:150,zIndex:2,background:'#fff6ee',borderRadius:20}}>{rolling?'读取中…':'✎ 捏小人'}</button>{error&&<p role="alert" style={{position:'absolute',left:18,top:195}}>{error}</p>}</section></>:<>
