...
export interface RollResult { layers:Record<string,string>; image:string }
export function CreatorRollBridge({ request, onReady, onResult, onError }: { request:number; onReady:()=>void; onResult:(r:RollResult)=>void; onError:(message:string)=>void }) {
    ...
    useEffect(()=>{
        ...
        window.addEventListener('message',async e=>{
          ...
          try{
            ...
            parent.postMessage({type:'experiment-roll-result',id,payload:{layers,image:whole.toDataURL()}},${JSON.stringify(origin)});
        ...
        fetch('/like520/character_creator.html').then(r=>{if(!r.ok)throw new Error('捏人器加载失败');return r.text();}).then(text=>{
            if(cancelled)return;
            // No changes to the production creator file; isolate its draft writes.
            setHtml(text.replace(/saveDraft\(\);/g,'/* preview: no draft writes */').replace(/<script\b[^>]*\bsrc=[^>]*>[\s\S]*?<\/script>/gi,'').replace('<head>','<head><base href="/like520/">').replace('</body>',bridge+'</body>'));
    ...
    useEffect(()=>{
        ...
        frame.current?.contentWindow?.postMessage({type:'experiment-roll',id:request},location.origin);
    },[request]);
