...
export function HairEditor({parts,hair,previewHair,assets,onChange,onUndo,onRedo,onReset,canUndo,canRedo,onBegin,onEnd}:{parts:Parts;hair:HairSettings;previewHair:HairSettings;assets:Record<string,string>;onChange:(v:HairSettings)=>void;onUndo:()=>void;onRedo:()=>void;onReset:()=>void;canUndo:boolean;canRedo:boolean;onBegin:()=>void;onEnd:()=>void}){
 ...
 useEffect(()=>{if(!['fronthair','earhair','back1','back2'].includes(selected)&&!hair.extras.some(e=>e.id===selected))setSelected('back2');},[hair.extras,selected]);
 ...
 return <div className="hair-editor">
  ...
  <div className="hair-options">
   ...
   <label>素材分类 <select aria-label="素材分类" value={extra?.mode??hairMode({...hair,assets},selected)} onChange={e=>{const mode=e.target.value as 'wrap'|'project';if(extra||!assets[selected])update({mode});else onChange({...hair,assetModes:{...hair.assetModes,[assets[selected]]:mode}});}}><option value="wrap">贴头包裹</option><option value="project">向外伸出（马尾 / 啾啾 / 猫耳）</option></select></label>
   <p>{extra?'分类随这张额外发片保存。':assets[selected]?`当前素材：${assets[selected]} · 分类只作用于这张素材。`:'当前层未选择素材。'}「全部重置」恢复默认分类与尺寸，并移除额外层；可以撤销。</p>
   {([['length','长度',.25,2],['width','宽度',.4,2],['offsetY','上下位置',-.8,.8],['distance','离头距离',0,.8]] as const).map(([key,label,min,max])=><label key={key}>{label}<input aria-label={`${label}调节`} type="range" min={min} max={max} step={.01} onPointerDown={onBegin} onPointerUp={onEnd} onPointerCancel={onEnd} onBlur={onEnd} value={layer[key]} onChange={e=>update({[key]:+e.target.value})}/><output>{layer[key].toFixed(2)}</output></label>)}
   ...
   <p>包裹类形成外圈，伸出类保留原图展开轮廓，透明处仍然透空。上传素材建议用与原图对齐的透明 PNG。</p>
