...
- 主依据使用个人陪伴 VPS 心跳快照里的角色当日日程、当前活动、下一活动和角色内心状态。
...
接入个人陪伴 VPS 心跳，不依赖主动消息 2.0：

1. 陪伴后端在每个角色自己的心跳中先检查 pending、24 小时滚动上限与 6 小时冷却，再让 LLM 结合人设、日程、经历和最近饮食愿望生成结构化意图。
2. 陪伴后端把意图写入该角色自己的 outbox；一个未确认意图会阻止同角色继续产生新意图。
3. 客户端再次验证角色、地址、银行卡、余额、冷却和商品，然后调用现有的原子结账方法。
4. 订单、扣款和账本只在客户端验证成功后落库，并把 placed / rejected 结果幂等回报给 VPS。

主要接点：

- `server/companion/src/heartbeat.mjs`、`delivery.mjs`、`store.mjs`：心跳决策、云端安全预筛与逐角色意图台账。
- `utils/companionBackendClient.ts`：上传去敏的地址标签、菜单、日程和愿望信号，并补收 outbox。
- `utils/companionBackendRuntime.ts`、`utils/deliveryAutonomy.ts`：接收、二次校验、原子结账和结果回执。
...
> 请继续维护 `kiko522/copie` 的 `master`。先完整阅读根目录 `AGENTS.md`、README 的“给想二改的人”和 `docs/commerce-vps-handoff-2026-09-22.md`，再检查最近提交与工作树，不要按摘要猜代码。角色自主点外卖接在个人陪伴 VPS 心跳，不依赖主动消息 2.0；不同角色的额度、冷却和数据必须完全隔离；每角色滚动 24 小时最多 2 单、至少间隔 6 小时，这只是上限不是每日任务；不要机械催早餐午餐晚餐；综合角色日程、当前活动、用户近 90 分钟仍有效的愿望和角色人设。云端只产生结构化意图，本地二次校验并调用现有原子结账，必须幂等，不能在成功前宣称已下单。先不要部署或重启 VPS，也不要要求或输出任何密钥。
