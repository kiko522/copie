...
统一管理一只角色在两处的 Q 版形象：**小小窝**房间立绘、**特别时光** 520 大头贴。两处可以单独捏（互不影响），也可以挑一处形象同步到另一处。入口在 神经链接 → 角色详情 → **「手办」tab**，全屏工坊 UI 走手办展示柜风。
...
捏人器完整导出 state（选件 + 换色 + 翻转 + 眼型…）按槽位存 `char.chibiStudio.{room,like520}.state`（`types.ts` 的 `ChibiStudioData`），再编辑时整套还原。`chibiStudio` 属运行时本地状态，已加入 `CARD_STRIPPED_FIELDS`（角色卡导出/导入双向剥离）。
...
- `chibiStudio.like520.img`（兜底大头贴 dataURL）、`specialMomentRecords…charChibi.dataUrl` 会被抽进 zip `assets/*`、导入时原样还原；
...
**媒体与美化素材（media_only）**模式的角色只导出一份手挑的视觉子集（avatar/sprites/roomItems/backgrounds…），不含 `chibiStudio`/`specialMomentRecords`——与这些运行时字段既有的处理一致，官方也提示「别只导媒体包」。回归测试见 `utils/backupExport.test.ts`。

角色**卡**分享（单角色导出）则会剥掉 `chibiStudio`（已在 `CARD_STRIPPED_FIELDS`），与 `specialMomentRecords` 同属运行时本地状态，不随卡外传。
